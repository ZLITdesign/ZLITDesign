!function (a) {
  ...
  a.fn.circleChart = function (b) {
    ...
    d.prototype = {
      onDraw(a) {
        if (!1 !== this.settings.onDraw) {
          ...
          if (Math.round(c.value)==100){  //百分百的颜色
            c.settings.color = '#87D068';
          }
          if (Math.round(c.value)<0){  //失败的颜色
            c.settings.color = '#EF5306';
          }
        }
      }, drawBackground(a) {
        //a.lineWidth  进度条宽度
        a.beginPath(), a.arc(this.pos, this.pos, this.settings.backgroundFix ? .9999 * this.radius : this.radius, 0, 2 * Math.PI), a.lineWidth = this.settings.backgroundFix ? .7 * this.lineWidth : this.lineWidth, a.strokeStyle = this.settings.backgroundColor, a.stroke()
      }, draw(a) {
        ...
        a.lineWidth = .7 * this.lineWidth, a.lineCap = this.settings.lineCap, a.strokeStyle = this.settings.color, a.stroke()
      }, animate(a, b, c, d, e) {
    ...
    let e = (b, c) => {
      ...
      }, f = a => a / Math.PI * 180, g = a => a / 180 * Math.PI, h = a => g(a / 100 * 360), i = a => f(a) / 360 * 100,
