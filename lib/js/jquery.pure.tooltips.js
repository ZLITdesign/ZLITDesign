...
(function($) {
  ...
  //白色背景
  $.pt = $.pureToolTips1 = function(options) {
    var opts = $.extend({
        ...
        autoClosetTime 		: 3000,		//鼠标停留在目标元素身上时自动关闭延时时长
        leaveClose 	: false,	//提示框失去焦点后关闭
				leaveCloseTime  : null,    //鼠标离开目标元素时关闭延时时长
        close 		: null,		//关闭回调函数
        fontcolor : null,   //字体颜色
				conAlign : 'center'  //文字内容对齐方式,默认剧中对齐
      }, options || {}),
    ...
    function init() {
      ...
      if (!opts.fontcolor){
        $pt.removeClass().addClass('pt ' + (arrow ? arrowClass : '')).find('.cont').html(opts.content).css({width:opts.width, height:opts.height, textAlign:opts.conAlign});
      }else{
        $pt.removeClass().addClass('pt ' + (arrow ? arrowClass : '')).find('.cont').html(opts.content).css({width:opts.width, height:opts.height, color:opts.fontcolor, textAlign:opts.conAlign});
      }
      ...
      opts.autoClose && !opts.leaveClose && autoClose();	//默认自动关闭，优先离开关闭
      return $pt;
    }
    //计算提示框应该出现在目标元素位置
    function setPos() {
  ...
  //扩展到包装集上
  $.fn.pt = $.fn.pureToolTips1 = function(options) {
    var opts = $.extend({
      leaveClose:true,
      leaveCloseTime:500
    }, options || {});
  ...
  //深色背景
  $.dt = $.pureToolTips1 = function(options) {
    var opts = $.extend({
        target 		: null,		//目标元素，不能为空
        position 	: 't',		//提示框相对目标元素位置 t=top,b=bottom,r=right,l=left
        align		: 'c',		//提示框与目标元素的对齐方式，自动调节箭头显示位置，指向目标元素中间位置，c=center, t=top, b=bottom, l=left, r=right [postion=t|b时，align=l|r有效][position=t|b时，align=t|d有效]
      ...
      constant = {b:'dt-up', t:'dt-down', r:'dt-left', l:'dt-right'}, //相对位置正好和箭头方向相反
      arrowClass = constant[position] || constant.t;
    ...
    function init() {
      ...
      if(!$dt.length) {
        $dt = $('<div class="dt dt-down"><div class="cont"></div><b class="out"></b><b class="in"></b></div>').appendTo(document.body);
      }
      if (!opts.fontcolor){
        $dt.removeClass().addClass('dt ' + (arrow ? arrowClass : '')).find('.cont').html(opts.content).css({width:opts.width, height:opts.height, textAlign:opts.conAlign});
      }else{
        $dt.removeClass().addClass('dt ' + (arrow ? arrowClass : '')).find('.cont').html(opts.content).css({width:opts.width, height:opts.height, color:opts.fontcolor, textAlign:opts.conAlign});
      }
      $ao = $dt.find('.out').toggle(arrow);
      $ai =  $dt.find('.in').toggle(arrow);
      w = $dt.outerWidth();
      h = $dt.outerHeight();
      arrow && autoAdjust();			//设置箭头自动居中
      $dt.css(setPos()).show();		//设置显示框位置和自动隐藏事件
      opts.leaveClose && leaveClose();//离开关闭
      opts.autoClose && !opts.leaveClose && autoClose();	//默认自动关闭，优先离开关闭
      return $dt;
    }
    ...
    function leaveClose() {
      //先解绑再绑定，不然会形成事件链
      $dt.unbind('mouseleave').mouseleave(function(e) {
        $dt.hide();
        $.isFunction(opts.close) && opts.close();
      }).unbind('mouseenter').mouseenter(function() {
        window.ptt && clearTimeout(ptt);
  ...
  //扩展到包装集上
  $.fn.dt = $.fn.pureToolTips1 = function(options) {
    var opts = $.extend({
      leaveClose:true,
      leaveCloseTime:500
    }, options || {});
